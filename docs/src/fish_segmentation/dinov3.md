...
4. Heatmap: `compute_anomaly_heatmap(tokens, grid_shape, target_shape,
   max_sample_pool=1500, border_margin_pct=0.05)` — 1 − mean cosine similarity per
   token (subsampling the reference pool keeps O(N·1500) instead of O(N²)), border
   tokens zeroed (ViT border artifact), bicubic upsample to pixels, min-max normalized.
